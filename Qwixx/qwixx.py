...
class InadequateChecks(Exception):
    def __init__(
        self,
        message="You are unable to select this cell. You need to have at least 5 checked boxes in this row",
    ):
        self.message = message
        super().__init__(self.message)
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        for hit_box, die_id in game.dice_hit_boxes:
                            if hit_box.collidepoint(x, y):
                                ...
                                if None not in game.selected_dice:
                                    self.qwixx_card.select_valid_cells(
                                        game.dice[game.selected_dice[0]],
                                        game.dice[game.selected_dice[1]],
                                    )
                                game.draw_game()
                    if self.qwixx_card.cell_hit_boxes:
                        for hit_box, cell in self.qwixx_card.cell_hit_boxes:
                            if hit_box.collidepoint(x, y):
                                ...
                                self.qwixx_card.selected_cell = (
                                    cell  # Update selected_cell with coordinates
                                )
                                game.draw_game()
                    ...
                    if event.key == pygame.K_RETURN:
                        if (
                            self.qwixx_card.selected_cell
                            and self.qwixx_card.selected_cell
                            in self.qwixx_card.valid_cells
                        ):
                            try:
...
class Cell:
    ...
    def draw_cell(self, screen):
        ...
        pygame.draw.rect(
            screen, self.rgb, (self.x + 10, self.y, CELL_WIDTH, CELL_HEIGHT)
        )
...
class Row:
    def __init__(
        ...
        self.cell_objects = {
            cell_index: Cell(
                ...
                rgb,
                string_color,
                x_cord + SPACE_BETWEEN_CELLS * cell_index,
                y_cord,
            )
            for cell_index, value in enumerate(
                range(row_start_index, row_end_index, reverse)
            )
        }
    ...
    def display_row(self, screen):
        ...
        for i, cell in enumerate(self.cells):
            cell_hit_box = pygame.Rect(
                cell.x + 100, cell.y + 200, CELL_WIDTH, CELL_HEIGHT
            )
    ...
    def is_mouse_over_cell(self, x, y):
        for cell in self.cells:
            ...
            if (
                cell_x <= x <= cell_x + CELL_WIDTH
                and cell_y <= y <= cell_y + CELL_HEIGHT
            ):
                return cell
        return None
...
class QwixxCard:
    ...
    def cross_out_cell(self):
        ...
        for row in self.board:
            for cell in row.cells:
                if self.selected_cell == cell:
                    curr_row = cell.row
                    r_index = self.get_right_most_crossed_off_box_in_row(
                        self.board[curr_row]
                    )
                    index = cell.column
        ...
        if HUMAN_TEST:
            print(
                f"{self.player_name}crossed out cell: ",
                self.selected_cell.row,
                self.selected_cell.column,
        ...
        if self.selected_cell is not None:
            ...
            pygame.draw.rect(
                ...
                (
                    x - border_thickness,
                    y - border_thickness,
                    CELL_WIDTH + (2 * border_thickness),
                    CELL_HEIGHT + (2 * border_thickness),
                ),
                border_thickness,
            )
    ...
    def get_play_card_surface(self):
        ...
        for row in self.board:
            ...
            hit_boxes = list(
                map(
                    lambda hit_box: (
                        hit_box[0].move(self.player_offset, 0),
                        hit_box[1],
                    ),
                    hit_boxes,
                )
...
class QwixxGame:
    ...
    def draw_dice(self, screen):
        ...
        for die in self.dice:
            if die.get_value() is not None:
                ...
                # hit box
                dice_hit_box = pygame.Rect(
                    400 + die.get_die_id() * 150, 650, DICE_WIDTH, DICE_HEIGHT
                )
    ...
    def draw_game(self):
        if DO_DRAW:
            ...
            instruction_label = INSTRUCTION_FONT.render(
                "INSTRUCTION: " + self.instruction_text, True, BLACK, WHITE
            )

            warning_label = FONT.render(self.warning_text, True, BLACK)

            game_state_label = FONT.render(
                f"STAGE: {self.game_state_text}", True, BLACK
            )
    ...
    def run(self):
        ...
        for i in range(len(self.players)):
            self.players[i].set_player_surface(
                SCREEN_WIDTH // len(self.players), SCREEN_HEIGHT - 200, player_colors[i]
            )
        ...
        while running:
            ...
            if self.game_state == GameState.ROLL_DICE:
                ...
                self.instruction_text = (
                    "Click valid cell. Press RETURN to confirm. Press SPACEBAR to skip."
                )
